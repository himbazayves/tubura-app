...
<nav aria-label="breadcrumb" role="navigation">
    ...
    <li class="breadcrumb-item"><a href="{{ url()->previous() }}">Years</a></li>
...
<form method="post" action="{{route('years.update',['year'=>$year->id])}}" >
    @csrf
    @method('PUT')
        ...
        <div class="col-sm-12 col-md-10">
                <input class="form-control String"  type="text"  name="name" id="name" value="{{old('name',$year->name)}}"
                        required="required"
                >
            @if($errors->has('name'))
